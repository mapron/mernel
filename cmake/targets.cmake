...
function(MakeTargetExport name)
    ...
    configure_file(${MERNEL_ROOT}/cmake/ExportLib.h.in ${headerName} @ONLY)
    target_sources(${name} PRIVATE ${headerName})
endfunction()

function(MakeTargetStaticExport name)
    string(TOUPPER "${name}_EXPORT" DEFINITION_NAME)
    set(TARGET ${name})
    set(headerName  ${CMAKE_CURRENT_BINARY_DIR}/export/${name}Export.hpp)
    configure_file(${MERNEL_ROOT}/cmake/StaticExportLib.h.in ${headerName} @ONLY)
    target_sources(${name} PRIVATE ${headerName})
endfunction()

# type = shared|static|interface|app_ui|app_qt|app_console|app_bundle|header_only
function(MakeTarget name type excludeAll)
    ...
    elseif(type STREQUAL app_qt)
        if (WIN32)
            qt_add_executable(${name} WIN32 ${excludeAll})
        else()
            qt_add_executable(${name} ${excludeAll})
        endif()
...
function(MakeTargetSources name skipGlob includes sourceDir useQt generateStub extraSources extraGlob excludeSources extraPostprocess)
    ...
    if (generateStub)
        ...
        endforeach()
        configure_file(${MERNEL_ROOT}/cmake/headerOnlyStub.cpp.in
            ${stubName})
...
function(AddTarget)
    ...
    foreach(qt ${ARG_QT_MODULES})
        target_link_libraries(${name} ${defaultVisibility} Qt6::${qt})
    endforeach()
    if (ARG_LINK_FLAGS)
        set_target_properties(${name} PROPERTIES LINK_FLAGS "${ARG_LINK_FLAGS}")
    endif()
    
    set(installableTypes shared app_ui app_qt app_console app_bundle)
